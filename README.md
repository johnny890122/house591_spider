...
```bash
house$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
house$ sam local start-api
house$ curl http://localhost:3000/classify_digit
```
...
```bash
house$ sam logs -n InferenceFunction --stack-name "house" --tail
```
